...
class BaseViewController: UIViewController {
        ...
        if self.parent!.isKind(of: UINavigationController.self) {
            ...
            if self.isNavigationBarShadowDisable() {
                ...
                self.navigationController?.navigationBar.titleTextAttributes = [
                    ...
                    NSFontAttributeName:UIFont.init(name: "HelveticaNeue", size: 17.0)!
                ]
            }
    ...
    func handleNetworkError(error:NSError){
        ...
        if error.domain == NSCocoaErrorDomain {
            ...
            let alert:PMAlertController = PMAlertController(title: "", description: NSLocalizedString(" Error on \(errorKey)\rFailure Reason:\(reason)", comment: ""), image: nil, style: .alert)
    ...
    func backBarButtonItem() -> UIBarButtonItem {
        ...
        button.imageEdgeInsets = UIEdgeInsetsMake(0, -16.0, 0, 0)
        return item
    }
...
//                _cartBarButtonItem?.tag =
    ...
    func badgeBarButtonItemWithButton(button:UIButton) -> BBBadgeBarButtonItem {
        ...
        item.style = .plain
        item.badgeBGColor = UIColor.init(hex: "db5a27")
        item.badgePadding = 4
